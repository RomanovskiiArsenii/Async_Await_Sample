...
class Program
{
    class MyClass
    {
        //асинхронный метод PrintAsync будет объявлен в начале метода Main, но
        //запущен на выполнение лишь в конце. Асинхронный метод Main дождется его завершения.
        public async Task PrintAsync()                                      //асинхронный метод
        {
            await Task.Run(async () =>                                      //асинхронная операция
            {
        ...
        public async Task<int> OperationAsync(CancellationToken token)      //асинхронный метод
        {
            ...
            return await Task.Run(async () =>                               //асинхронная операция
            {
                ...
                if(!token.IsCancellationRequested)
                    await Console.Out.WriteLineAsync("\nPress any key to continue");        //при завершении
